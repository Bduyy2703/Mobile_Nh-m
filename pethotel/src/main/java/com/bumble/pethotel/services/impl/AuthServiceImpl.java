...
import com.bumble.pethotel.security.JwtTokenProvider;
...
public class AuthServiceImpl implements AuthService {
    ...
    public AuthenticationResponse login(LoginDto loginDto) {
        ...
        if (!user.isEmailVerified()) {
            throw new PetApiException(HttpStatus.FORBIDDEN, "Email is not verified. Please verify your email.");
        }
    ...
    public AuthenticationResponse refreshToken(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if (username != null) {
            ...
            if (!token.isRevoked() && !token.isExpired()) {
                ...
                throw new PetApiException(HttpStatus.BAD_REQUEST, "Invalid refresh token");
            }
        }
        return null;
    }
    ...
    public String verifyEmailCode(String email, String code) {
        User user = userRepository.findByEmail(email)
                .orElseThrow(() -> new PetApiException(HttpStatus.BAD_REQUEST, "User not found with this email."));

        if (user.getVerificationCode() == null || !user.getVerificationCode().equals(code)) {
            throw new PetApiException(HttpStatus.BAD_REQUEST, "Invalid verification code.");
        }

        if (user.getVerificationCodeExpiry().isBefore(LocalDateTime.now())) {
            throw new PetApiException(HttpStatus.BAD_REQUEST, "Verification code has expired.");
        }
    ...
    public String resendVerificationCode(String email) {
        ...
        if (user.isEmailVerified()) {
            throw new PetApiException(HttpStatus.BAD_REQUEST,"Email is already verified.");
        }
        emailVerificationService.sendVerificationCode(user);
